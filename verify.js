  ...
  const getReq = https.request(getOptions, (res) => {
    ...
    res.on('end', () => {
      ...
      try {
        ...
        // Step 2: Find user and update email_verified to true
        ...
        const putPayload = JSON.stringify({
          message: `Verify email for ${email} via Auth0`,
        ...
        const putReq = https.request(putOptions, (putRes) => {
          ...
          putRes.on('end', () => {
            if (putRes.statusCode < 200 || putRes.statusCode >= 300) {
              return callback(new Error(`Failed to commit email verification to GitHub (HTTP ${putRes.statusCode}): ${putBody}`));
            }
            // Return true indicating the update succeeded
        ...
        return callback(new Error('Failed during email verification processing: ' + err.message));
      }
    });
  });
